...
function Clothes() {
  ...
  useEffect(() => {
    if (weather.main) {
      ...
      const weatherTypeVar = weatherId >= 200 && weatherId <= 531 ? 'rain'
        : weatherId >= 600 && weatherId <= 622 ? 'snow'
          : weatherId === 800 ? 'clear' : 'clouds';

      const temperatureTypeVar = temperature < -20 ? 'extraFreeze'
        : temperature >= -20 && temperature < 1 ? 'freeze'
          : temperature >= 1 && temperature < 11 ? 'cold'
            : temperature >= 11 && temperature < 21 ? 'warm' : 'hot';

      setWeatherType(weatherTypeVar);
      setTemperatureType(temperatureTypeVar);
    }
  }, [weather]);
